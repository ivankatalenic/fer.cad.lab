...
static int find_highest(Simplex& simplex) {
	const int size{simplex.size()};
	if (size < 1) {
		throw std::invalid_argument("tried to find the simplex highest when the simplex has no vertices");
	}
...
static int find_second_highest(Simplex& simplex) {
	const int size{simplex.size()};
	if (size < 2) {
		throw std::invalid_argument("tried to find the simplex second highest when the simplex has less than two vertices");
	}
	...
	int second_best{(best + 1) % size};
	for (int i{(best + 2) % size}; i != best; i = (i + 1) % size) {
...
static Matrix centroid(Simplex& simplex) {
	const int size{simplex.size()};
	if (size < 1) {
		throw std::invalid_argument("can't find centroid of an empty simplex");
	}
...
Matrix Optimization::boxMethod(
	...
	if (!is_inside_bounds(bound_lower, bound_upper, start)
		...
		// TODO: Possible error: Could be inside_point(constr, ...).
	...
	for (int t{0}; t < 2 * n; ++t) {
		...
		while (!are_constraints_satisfied(constraints, p)) {
			// TODO: Prevent infinite loop here by checking if cent satisfies all constraints
			p = 0.5 * (p + cent);
		}
		simplex.append(p);
	...
	for (int i{0}; i < iter_max; ++i) {
		...
		while (!are_constraints_satisfied(constraints, xr)) {
			// TODO: Make sure centroid is in satisfiable area.
			xr = 0.5 * (xr + cent);
		}
